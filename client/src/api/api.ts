import { ServerResponse, PagesData, ApisData, StatsData, TablesData, PageNode, ApiCategory, TableInfo } from '@/types'

const API_BASE_URL = import.meta.env.VITE_API_URL || 'http://localhost:5000'

// Generic fetch wrapper
async function fetchApi<T>(endpoint: string): Promise<ServerResponse<T>> {
...
export const getTableByName = (tableName: string) =>
  fetchApi<TableInfo>(`/api/tables/${tableName}`)

export const getTablesByCategory = (category: string) =>
  fetchApi<TablesData>(`/api/tables/category/${category}`)

export const healthCheck = () => fetchApi<{ message: string }>('/api/health')